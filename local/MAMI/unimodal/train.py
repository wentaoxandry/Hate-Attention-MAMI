...
def processing(config, dataset=None):
    """
    Train a CLIP-based model (image or text) with early stopping and evaluate on the test set.
    ...
    hiddendim = 768
    if 'image' in config['modal']: 
        model = CLIPimage(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=hiddendim, task=config["task"])
        
    else:
        model = CLIPtext(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=hiddendim, task=config["task"])
    
    # freez clip text or image encoder to avoid catastrophic forgetting
    if 'image' in config['modal']:
...
if __name__ == '__main__':
    ...
    modeldir = os.path.join(savedir, 'Unimodal', modal, task, 'model')              # dir saves the trained model
    resultsdir = os.path.join(savedir, 'Unimodal', modal, task, 'results')          # dir saves the predicted test set results
    ...
    tokenizer = CLIPProcessor.from_pretrained(MODELtext, cache_dir=cashedir)        # load the pre-trained CLIP model tokenizer
