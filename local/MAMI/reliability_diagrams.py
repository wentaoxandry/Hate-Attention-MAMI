...
def compute_calibration(true_labels, pred_labels, confidences, num_bins=10):
    ...
    for b in range(num_bins):
        ...
        if len(selected) > 0:
            #a = true_labels[selected] == pred_labels[selected]
            bin_accuracies[b] = np.mean(true_labels[selected] == pred_labels[selected])
            #bin_confidences[b] = np.mean([confidences[i] for i in selected])
...
def _reliability_diagram_subplot(ax, bin_data, 
    ...
    if draw_ece:
        ...
        ax.text(0.98, 0.02, "ECE=%.2f" % ece, color="black", 
                ha="right", va="bottom", transform=ax.transAxes, size=fontsize)
    ...
    ax.legend(handles=[gap_plt, acc_plt], fontsize=fontsize)#, bbox_to_anchor=(1,0.5), loc="center right",)
...
def _confidence_histogram_subplot(ax, bin_data, 
    ...
    if draw_averages:
        ...
        ax.legend(handles=[acc_plt, conf_plt], fontsize=fontsize)


def _reliability_diagram_combined(bin_data, 
                                  draw_ece, draw_bin_importance, draw_averages, 
                                  title, imagesavedir, modal, figsize, dpi, return_fig):
    ...
    #plt.show()
    plt.savefig(os.path.join(imagesavedir, modal + '.pdf'), format="pdf", bbox_inches="tight")

    if return_fig: return fig


def reliability_diagram(true_labels, pred_labels, confidences, imagesavedir, modal, num_bins=10,
    ...
    return _reliability_diagram_combined(bin_data, draw_ece, draw_bin_importance,
                                         draw_averages, title, imagesavedir, modal, figsize=figsize, 
                                         dpi=dpi, return_fig=return_fig), bin_data["expected_calibration_error"]
def get_ece_score(true_labels, pred_labels, confidences, modal, num_bins=10,
                        draw_ece=True, draw_bin_importance=False, 
                        draw_averages=True, title="Reliability Diagram", 
                        figsize=(6, 6), dpi=72, return_fig=False):
    """Draws a reliability diagram and confidence histogram in a single plot.
