...
def processing(config, mconfig, dataset=None):
    """
    Train a Tiny-LLaVA with early stopping and evaluate on the test set.
    ...
    # create model
    model = LLAVA(odim=odim, modelname=config["MODELtext"],  mconfig=mconfig)
...
def get_args():
    ...
    parser.add_argument('--modal', default='Tiny-LLaVA', type=str, help='model type')
...
if __name__ == '__main__':
    ...
    for makedir in [modeldir, resultsdir]:                                        # create the folders if it not exist
    ...
    # the pre-trained Tiny-LLaVA model used
    MODELtext = "llava-hf/llava-1.5-7b-hf"

    # load the pre-trained Tiny-LLaVA model processor
    processor = AutoProcessor.from_pretrained(MODELtext)

    dataset = LLaVAdatasetclass(train_file=traindict,                                       # create dataset for training and evaluation
                                val_file=valdict,
                                test_file=testdict,
                                device=device,
                                task=task)
