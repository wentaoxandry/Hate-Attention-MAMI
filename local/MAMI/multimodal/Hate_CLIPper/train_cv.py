...
def get_args():
    ...
    parser.add_argument('--modal', default='Hate-CLIPper-bimodal', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    if task == 'taskA':
        ...
        #pretrainfold = os.path.join(savedir, 'Multimodal', modal, 'taskA', 'model')
        #pretrainame = find_best_trained(os.listdir(pretrainfold))
        #pretraineddir = os.path.join(pretrainfold, pretrainame[0])
        pretraineddir = None
    max_num_epochs = 30
    ...
    '''with open(os.path.join(datadir, "train_chatgpt.json"), encoding="utf8") as json_file:
        traindesdict = json.load(json_file)
    with open(os.path.join(datadir, "test_chatgpt.json"), encoding="utf8") as json_file:
        testdesdict = json.load(json_file)
    
    for dset, workdict in zip(['train', 'val', 'test'], [traindict, valdict, testdict]):
        if dset is not 'test':
            for i in list(workdict.keys()):
                deskey = i.split('_')[0]
                workdict[i].update({'caption': traindesdict[deskey]['caption']})
        else:
            for i in list(workdict.keys()):
                deskey = i.split('_')[0]
                workdict[i].update({'caption': testdesdict[deskey]['caption']})'''
    alldict = {}
    ...
    for cv in range(10):
        devlist = np.load(os.path.join(datadir, 'cv', str(cv) + '.npy'))
        traindict = {}
        valdict = {}
