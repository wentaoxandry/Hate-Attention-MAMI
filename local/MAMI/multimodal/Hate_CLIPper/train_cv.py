...
def run(data_loader, model, criterion, train=True, optimizer=None):
    """
...
def processing(config, mconfig, dataset=None):
    """
    Train a Hate-CLIPper with early stopping and evaluate on the test set.
    ...
    for epoch in range(config["epochs"]):  # loop over the dataset multiple times
        ...
        torch.cuda.empty_cache()
        if lossmean >= evalloss_best:
            stop_counter = stop_counter + 1
            print('no improvement')
            continuescore = 0
        else:
            print('new score')
            evalloss_best = lossmean
...
def get_args():
    ...
    parser.add_argument('--modal', default='Hate-CLIPper', type=str, help='which model')
    parser.add_argument('--task', default='taskB', type=str, help='which task, options: taskA, taskB')
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available() == True:                                           # set GPU if it is aviliable
        ...
        device = 'cpu'                                                              # define the maximum number of epochs
    ...
    # combine train and validation set, the combined set will be used to split for cross validation
    alldict = {}
    alldict.update(traindict)
    alldict.update(valdict)

    # the pre-trained CLIP model used
    MODELtext = "openai/clip-vit-large-patch14"
    max_len = 77                                                                    # maximum token length of the CLIP model                                       

    # load the pre-trained CLIP model tokenizer
    tokenizer = CLIPProcessor.from_pretrained(MODELtext, cache_dir=cashedir)

    # run cross-validation experiment. 
    for cv in range(10):
        # reconstruct train and validation set based on the pre-defined datei
        devlist = np.load(os.path.join(metadir, 'cv', str(cv) + '.npy'))
        traindict = {}
        valdict = {}
        for key in list(alldict.keys()):
            if key in devlist:
                valdict.update({key: alldict[key]})
            else:
                traindict.update({key: alldict[key]})

        # create the folders if it not exist
        modeldir = os.path.join(savedir, 'Multimodal', modal + '-cv', task, str(cv), 'model')
        resultsdir = os.path.join(savedir, 'Multimodal', modal + '-cv', task, str(cv), 'results')
