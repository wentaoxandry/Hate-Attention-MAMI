...
def pad_clip_custom_sequence(sequences):
    ...
    label_sequence = []
    filename_sequence = []
    for node_sets, mask, picel_values, score, labls, filename in sequences:
        ...
        score_squence.append(score.squeeze(0))
    ...
    score_squence = torch.nn.utils.rnn.pad_sequence(score_squence, batch_first=True)
    label_sequence = torch.nn.utils.rnn.pad_sequence(label_sequence, batch_first=True)
    return node_sets_sequence, mask_sequence, picel_values_sequence, score_squence, label_sequence, filename_sequence

class CLIPdatasetclass:  # This class is used to achieve parameters sharing among datasets
    def __init__(self, train_file, val_file, test_file, tokenizer, device, max_len, task=None, modal=None):
...
class CLIPdatasetloader(Dataset):  # For instantiating train, validation and test dataset
    ...
    def __getitem__(self, index):
        ...
        score = torch.FloatTensor(imagescore).unsqueeze(0)
        if self.task == 'taskA':
        ...
        return ids, mask, pixel_values, score, label, filename  # twtfsingdata.squeeze(0), filename
