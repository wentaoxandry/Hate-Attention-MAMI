...
def compute_scoreB(taskBdict):
  ...
  #print('Task B score is ' + str(round(score, 5)))
  return score
def find_file(filelist):
    data = [i for i in filelist if i.startswith('best')]
    return data
...
def get_args():
    ...
    parser.add_argument('--sourcedir', default='./../output', type=str, help='Dir saves the datasource information')
    parser.add_argument('--type', default='Multimodal', type=str, help='Dir saves the datasource information')
    parser.add_argument('--model', default='EarlymaskedLM_new', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    taskAdir = os.path.join(sourcedir, type, model, 'taskA', 'results')
    taskBdir = os.path.join(sourcedir, type, model, 'taskB', 'results')
    taskAfiledir = os.path.join(taskAdir, find_file(os.listdir(taskAdir))[0])
    taskBfiledir = os.path.join(taskBdir, find_file(os.listdir(taskBdir))[0])

    with open(taskAfiledir, encoding="utf8") as json_file:
        taskAdict = json.load(json_file)

    with open(taskBfiledir, encoding="utf8") as json_file:
        taskBdict = json.load(json_file)

    compute_scoreA(taskAdict)
    compute_scoreB(taskBdict)
    a = 1
