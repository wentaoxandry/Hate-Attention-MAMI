...
def training(config, dataset=None):
    ...
    if 'DSW' in config['modal']:
        model = DSW(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=32, RMs=config['RMs'])
    elif 'Representation' in config['modal']:
        model = Representation(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768, RMs=config['RMs'])
    ...
    for epoch in range(config["epochs"]):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(tqdm(data_loader_train), 0):
            ...
            labelA = data[4].to(config["device"])
            labelB1 = data[5].to(config["device"])
            labelB2 = data[6].to(config["device"])
            labelB3 = data[7].to(config["device"])
            labelB4 = data[8].to(config["device"])
            ...
            optimizer.zero_grad()
            outputs = model(inputs_values, attention_mask, pixel, score)
...
def get_args():
    ...
    parser.add_argument('--task', default='taskC', type=str, help='which data stream')
    parser.add_argument('--RMs', default='all_RMs', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    pretrainedtextdir = os.path.join(savedir, 'Unimodal', 'memotion_text', task, classname, 'model', 'bestmodel.pkl')
    pretrainedimgdir = os.path.join(savedir, 'Unimodal', 'memotion_image', task, classname, 'model', 'bestmodel.pkl')
    ...
    for dset, workdict, scoredicts in zip(['train', 'dev', 'test'], [traindict, valdict, testdict],
                                         [[texttrainscoredict, imagetrainscoredict],
                                        [textdevscoredict, imagedevscoredict],
                                          [texttestscoredict, imagetestscoredict]]):
        for key in list(workdict.keys()):
            workdict[key].update({'textscore': scoredicts[0][key]})
            workdict[key].update({'imagescore': scoredicts[1][key]})
        if dset == 'train':
            traindict = workdict
        elif dset == 'dev':
            valdict = workdict
