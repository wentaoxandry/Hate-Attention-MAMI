...
def pad_clip_custom_sequence(sequences):
    ...
    labelB4_sequence = []
    filename_sequence = []
    for node_sets, mask, picel_values, scores, labelA, labelB1, labelB2, labelB3, labelB4, filename in sequences:
        ...
        score_sequence.append(scores.squeeze(0))
    ...
    score_sequence = torch.nn.utils.rnn.pad_sequence(score_sequence, batch_first=True)
    ...
    return node_sets_sequence, mask_sequence, picel_values_sequence, score_sequence, labelA_sequence.squeeze(-1), labelB1_sequence.squeeze(-1), \
...
class CLIPdatasetloader(Dataset):  # For instantiating train, validation and test dataset
    ...
    def __getitem__(self, index):
        textscore = self.datadict[self.datakeys[index]]['textscore']
        if math.isnan(textscore):
            textscore = -1
        imagescore = copy.deepcopy(self.datadict[self.datakeys[index]]['imagescore'])
        imagescore.append(textscore)
        ...
        imagescore = torch.FloatTensor(imagescore)
        ...
        if ids[0].size()[0] > self.max_len:
            ...
            return newid, newmask, pixel_values, imagescore, labelA, labelB1, labelB2, labelB3, labelB4, filename
        else:
            return ids, mask, pixel_values, imagescore, labelA, labelB1, labelB2, labelB3, labelB4, filename
