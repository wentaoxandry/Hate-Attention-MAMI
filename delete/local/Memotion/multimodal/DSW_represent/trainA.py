...
def training(config, dataset=None):
    ...
    if 'DSW' in config['modal']:
        model = DSW(odim=3, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=32, RMs=config['RMs'])
    elif 'Representation' in config['modal']:
        model = Representation(odim=3, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768, RMs=config['RMs'])
...
def get_args():
    ...
    parser.add_argument('--RMs', default='no_RMs', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    pretrainedtextdir = os.path.join(savedir, 'Unimodal', 'memotion_text', task, 'model', 'bestmodel.pkl')
    pretrainedimgdir = os.path.join(savedir, 'Unimodal', 'memotion_image', task, 'model', 'bestmodel.pkl')
