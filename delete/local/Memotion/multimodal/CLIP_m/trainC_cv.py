...
if __name__ == '__main__':
    ...
    cvfiledir = os.path.join(datadir, 'CVs')
    if not os.path.exists(cvfiledir):
        os.makedirs(cvfiledir)
        import random
        ...
        for i in range(5):
            selectvalkeys = random.sample(keys, 1500)
            keys = list(set(keys) - set(selectvalkeys))
            np.save(os.path.join(cvfiledir, str(i) + '_dev.npy'), np.asarray(selectvalkeys))

    for i in range(5):
        modeldir = os.path.join(savedir, 'MultiCVmodal', modal, str(i), task, classname, 'model')
        resultsdir = os.path.join(savedir, 'MultiCVmodal', modal, str(i), task, classname, 'results')

        for makedir in [modeldir, resultsdir, cashedir]:
            if not os.path.exists(makedir):
                os.makedirs(makedir)
        devcvlist = np.load(os.path.join(cvfiledir, str(i) + '_dev.npy'))
        valdict = {str(key) + '_dev': val for key, val in valdict.items()}
        traindict.update(valdict)
        newtraindict = {}
        newvaldict = {}
        for key in list(traindict.keys()):
            if key in devcvlist:
                newvaldict.update({key: traindict[key]})
            else:
                newtraindict.update({key: traindict[key]})
