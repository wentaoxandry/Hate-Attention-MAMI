...
def training(config, dataset=None):
    ...
    model = CLIP_multi(odim=3, modelname=config["MODELtext"], cachedir=config["cachedir"])
...
if __name__ == '__main__':
    ...
    for i in range(5):
        modeldir = os.path.join(savedir, 'MultiCVmodal', modal, str(i), task, 'model')
        resultsdir = os.path.join(savedir, 'MultiCVmodal', modal, str(i), task, 'results')
