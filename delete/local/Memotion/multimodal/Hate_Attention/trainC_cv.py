...
def training(config, mconfig, dataset=None):
    ...
    model = CLIP_multi(odim=odim, config=config, mconfig=mconfig)
...
def get_args():
    ...
    parser.add_argument('--modal', default='Hate-Attention', type=str, help='which data stream')
    ...
    parser.add_argument('--config', default='config1', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    for cv in range(5):
        ...
        modeldir = os.path.join(savedir, 'Multimodal', modal + '-cv', task, classname, str(cv), 'model')
        resultsdir = os.path.join(savedir, 'Multimodal', modal + '-cv', task, classname, str(cv), 'results')
