...
def training(config, dataset=None):
    ...
    if 'text' in config['modal']:
        model = CLIPtext(odim=3, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768)
    else:
        model = CLIPimage(odim=3, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768)
...
def get_args():
    ...
    parser.add_argument('--modal', default='memotion_text', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    modeldir = os.path.join(savedir, 'Unimodal', modal, task, 'model')
    resultsdir = os.path.join(savedir, 'Unimodal', modal, task, 'results')
