...
def training(config, dataset=None):
    ...
    if 'text' in config['modal']:
        model = CLIPtext(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768)
    else:
        model = CLIPimage(odim=odim, modelname=config["MODELtext"], cachedir=config["cachedir"], hiddendim=768)
    ...
    for name, param in model.named_parameters():
        if 'clip' in name:  # Adjust this condition to match your CLIP model's parameter names
            clip_params.append(param)
        else:
            other_params.append(param)

    # Define different learning rates
    lr_clip = 1e-5  # lower learning rate for CLIP model parameters
    lr_other = config["lr"]  # higher learning rate for other model parameters

    # Create the optimizer with parameter groups
    optimizer = torch.optim.AdamW([
        {'params': clip_params, 'lr': lr_clip},
...
def get_args():
    ...
    parser.add_argument('--modal', default='memotion_image', type=str, help='which data stream')
...
if __name__ == '__main__':
    ...
    modal = args.modal + '_fine-tune'
    cashedir = args.cashedir

    modeldir = os.path.join(savedir, 'Unimodal', modal, task, classname, 'model')
    resultsdir = os.path.join(savedir, 'Unimodal', modal, task, classname, 'results')
